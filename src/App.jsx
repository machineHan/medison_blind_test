...
const metrics = [
  { id: "artifact", label: "아티팩트 개선", full: "3D 영상 특유의 패턴 아티팩트가 완화되었나요?", scale: ["전혀 완화되지 않음", "소폭 완화", "다소 완화", "확연히 완화"] },
  { id: "hallucination", label: "할루시네이션 개선", full: "영상 내 실제 존재하지 않는 구조가 관찰되었나요?", scale: ["명확하게 관찰됨", "다소 관찰됨", "경미하게 관찰됨", "관찰되지 않음"] },
  { id: "resolution", label: "해상도 개선", full: "영상의 전반적인 해상도와 세부 구조 표현이 개선되었나요?", scale: ["전혀 개선되지 않음", "소폭 개선", "다소 개선", "확연히 개선"] },
  { id: "contrast", label: "Contrast 개선", full: "영상 전반의 대비가 개선되었나요?", scale: ["전혀 개선되지 않음", "소폭 개선", "다소 개선", "확연히 개선"] },
  { id: "noise", label: "노이즈 개선", full: "영상 전반의 노이즈가 감소하였나요?", scale: ["전혀 감소하지 않음", "소폭 감소", "다소 감소", "확연히 감소"] },
];
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-slate-300 bg-white shadow-sm sm:rounded-[1.5rem]">
      <div className="flex items-center justify-between gap-2 border-b border-slate-300 bg-white px-3 py-2 sm:px-5 sm:py-4">
        ...
        <div className="text-right text-[11px] font-black text-slate-400 sm:text-sm">후보별 5개 항목 평가</div>
      </div>
        ...
        <thead>
          <tr>
            ...
            {candidateList.map((candidate) => (
              <th key={candidate.id} className="border-b border-r border-slate-300 bg-white px-1 py-3 text-sm font-black text-slate-950 last:border-r-0 sm:px-3 sm:py-6 sm:text-3xl">
                {candidate.label}
              </th>
            ))}
          </tr>
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[1.08fr_0.92fr] lg:px-8 lg:py-12">
          ...
          <div className="space-y-5 rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm sm:p-8">
            {metrics.map((metric, index) => (
              <div key={metric.id} className="border-b border-slate-100 pb-5 last:border-0 last:pb-0">
                <h2 className="text-lg font-black leading-snug text-slate-950 sm:text-2xl">{index + 1}. {metric.full}</h2>
        ...
        <div className="self-center rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-xl shadow-slate-200/70 sm:rounded-[2rem] sm:p-7">
          ...
          <h2 className="text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">3 Candidates × 5 Metrics</h2>
          <p className="mt-3 text-base leading-7 text-slate-600">Candidate 1, 2, 3의 품질을 항목별로 0~3점 평가함.</p>
          ...
          <button onClick={onStart} className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-[#0f5d75] px-5 py-4 text-lg font-black text-white shadow-lg shadow-[#0f5d75]/20">
            평가 시작 <ChevronRight size={20} />
          </button>

          <button onClick={onResults} className="mt-3 flex w-full items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-5 py-4 text-base font-black text-slate-700">
...
function EvaluationScreen({ evaluatorId, answers, setAnswers, onResults, onReset, onParticipantAnswers }) {
  ...
  const onScore = async (questionId, candidateId, metricId, score) => {
    ...
    try {
      ...
      console.error(error);
      alert("점수 저장 중 오류가 발생했습니다. Firebase 연결과 Firestore Rules를 확인해주세요.");
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      <div className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-2 py-2 sm:px-6 sm:py-4">
          ...
          <div className="flex shrink-0 items-center gap-1.5 sm:gap-3">
            <div className="hidden rounded-2xl bg-slate-50 px-4 py-3 text-sm font-black text-slate-600 sm:block">전체 {progress}% 완료</div>
            <div className="rounded-xl bg-slate-50 px-2.5 py-2 text-[11px] font-black text-slate-600 sm:rounded-2xl sm:px-4 sm:py-3 sm:text-sm">{currentComplete}/{currentTotal}</div>
            <div className="hidden rounded-2xl bg-slate-50 px-4 py-3 text-sm font-black text-slate-500 sm:block">{isSaving ? "저장 중" : "저장됨"}</div>
      ...
      <section className="mx-auto max-w-7xl px-1.5 py-2 sm:px-6 sm:py-6">
        ...
        <div className="mt-3 flex items-center justify-between pb-20 sm:justify-end sm:gap-3 sm:pb-0">
          <button onClick={() => goToQuestion(Math.max(0, currentIndex - 1))} className="flex items-center gap-1 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-black text-slate-700 disabled:opacity-40 sm:rounded-2xl sm:px-5 sm:py-3 sm:text-base" disabled={currentIndex === 0}>
            <ChevronLeft size={16} /> 이전
          </button>
          <button onClick={() => goToQuestion(currentIndex + 1)} className="fixed bottom-4 right-4 z-30 flex items-center gap-1 rounded-2xl bg-[#0f5d75] px-5 py-3 text-sm font-black text-white shadow-xl shadow-[#0f5d75]/25 disabled:opacity-40 sm:static sm:px-5 sm:py-3 sm:text-base">
            {currentIndex === questions.length - 1 ? "완료" : "다음"} <ChevronRight size={16} />
          </button>
        </div>
      </section>
    </main>
...
function TotalRankCards({ result }) {
  const sortedTotal = [...result.totalByModel].sort((a, b) => b.average - a.average);

  return (
    <div className="grid gap-5 lg:grid-cols-3">
      {sortedTotal.map((item, index) => (
        <div key={item.method} className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-black uppercase tracking-wider text-slate-400">Rank {index + 1}</p>
              <h2 className="mt-1 text-2xl font-black text-slate-950">{item.method}</h2>
            </div>
            {index === 0 ? <Trophy className="text-[#0f5d75]" /> : <BarChart3 className="text-slate-300" />}
          </div>
          <p className="mt-6 text-5xl font-black tracking-tight text-slate-950">{item.average.toFixed(2)}</p>
          <p className="mt-1 text-sm font-bold text-slate-500">/ 3.00 · {item.count} scores</p>
          <div className="mt-5"><ScoreBar value={item.average} /></div>
        </div>
      ))}
    </div>
  );
        ...
        {result.metricRows.map((row) => {
          ...
          return (
            <div key={row.metric.id} className="rounded-2xl bg-slate-50 p-4">
              <p className="mb-3 text-sm font-black text-slate-700">{row.metric.label}</p>
                ...
                {ranking.map((score, index) => (
                  <div key={score.method} className="flex items-center justify-between rounded-xl bg-white px-3 py-2 ring-1 ring-slate-100">
                    ...
                    <div className="text-right">
                      ...
                      <span className="ml-1 text-xs font-bold text-slate-400">({score.count})</span>
                    </div>
                  </div>
                ))}
              </div>
...
function ResultsScreen({ participants, currentEvaluatorId, currentAnswers, onBack, onParticipantView }) {
  const [scope, setScope] = useState("all");
  const currentParticipant = { evaluatorId: currentEvaluatorId || "current_rater", answers: currentAnswers || {} };
      ...
      <section className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <button onClick={() => setScope("all")} className={cn("rounded-2xl px-4 py-3 text-sm font-black", scope === "all" ? "bg-slate-950 text-white" : "bg-white text-slate-600 ring-1 ring-slate-200")}>전체 응답</button>
            <button onClick={() => setScope("current")} className={cn("rounded-2xl px-4 py-3 text-sm font-black", scope === "current" ? "bg-slate-950 text-white" : "bg-white text-slate-600 ring-1 ring-slate-200")}>현재 응답만</button>
            ...
            <button onClick={onParticipantView} className="flex items-center gap-2 rounded-2xl bg-[#0f5d75] px-4 py-3 text-sm font-black text-white shadow-lg shadow-[#0f5d75]/20">
              <Users size={17} /> 참여자별 보기
            </button>
          </div>
        </div>

        <div className="space-y-6">
          <TotalRankCards result={result} />
          <MetricRankList result={result} />
        </div>
      </section>
    </main>
  );
...
function ParticipantDetailScreen({ participants, currentEvaluatorId, currentAnswers, onBack, onResults }) {
  const participantOptions = useMemo(() => {
    const map = new Map();
    participants.forEach((participant) => {
      const key = participant.evaluatorKey || participant.evaluatorId;
      if (key) map.set(key, participant);
    });
    if (currentEvaluatorId) {
      const key = encodeURIComponent(String(currentEvaluatorId).trim().replace(/\s+/g, "_").toLowerCase());
      if (!map.has(key)) map.set(key, { evaluatorId: currentEvaluatorId, evaluatorKey: key, answers: currentAnswers || {} });
    }
    return [...map.values()].sort((a, b) => String(a.evaluatorId || "").localeCompare(String(b.evaluatorId || "")));
  }, [participants, currentEvaluatorId, currentAnswers]);

  const [selectedKey, setSelectedKey] = useState("");

  useEffect(() => {
    if (!selectedKey && participantOptions.length > 0) {
      setSelectedKey(participantOptions[0].evaluatorKey || participantOptions[0].evaluatorId);
    }
  }, [participantOptions, selectedKey]);

  const selectedParticipant = participantOptions.find((item) => (item.evaluatorKey || item.evaluatorId) === selectedKey);
  const selectedResult = useMemo(() => aggregate(selectedParticipant ? [selectedParticipant] : []), [selectedParticipant]);
  const completedScores = countAllScores(selectedParticipant?.answers || {});
  const totalScores = questions.length * candidateList.length * metrics.length;
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <section className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.25em] text-[#0f5d75]">Participant Detail</p>
            <h1 className="text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">참여자별 평가 내역</h1>
            <p className="mt-2 text-sm font-bold text-slate-500">선택 참여자 기준으로 전체 모델 순위와 항목별 점수 나열을 확인함.</p>
          </div>

          <div className="flex flex-wrap gap-2">
            <button onClick={onResults} className="rounded-2xl bg-slate-950 px-4 py-3 text-sm font-black text-white">전체 결과</button>
            <button onClick={onBack} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-black text-slate-600">평가로 돌아가기</button>
          </div>
        </div>

        <div className="mb-6 rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
          <label className="block text-sm font-black text-slate-700">참여자 선택</label>
          <select
            value={selectedKey}
            onChange={(event) => setSelectedKey(event.target.value)}
            className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-base font-black text-slate-800 outline-none focus:border-[#0f5d75] focus:bg-white"
          >
            {participantOptions.length === 0 && <option value="">참여자 없음</option>}
            {participantOptions.map((participant) => (
              <option key={participant.evaluatorKey || participant.evaluatorId} value={participant.evaluatorKey || participant.evaluatorId}>
                {participant.evaluatorId || participant.evaluatorKey}
              </option>
            ))}
          </select>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <div className="rounded-2xl bg-slate-50 p-4">
              <p className="text-xs font-black uppercase tracking-wider text-slate-400">Evaluator</p>
              <p className="mt-1 text-xl font-black text-slate-950">{selectedParticipant?.evaluatorId || "-"}</p>
            </div>
            <div className="rounded-2xl bg-slate-50 p-4">
              <p className="text-xs font-black uppercase tracking-wider text-slate-400">Progress</p>
              <p className="mt-1 text-xl font-black text-slate-950">{completedScores}/{totalScores}</p>
            </div>
            <div className="rounded-2xl bg-slate-50 p-4">
              <p className="text-xs font-black uppercase tracking-wider text-slate-400">Completed</p>
              <p className="mt-1 text-xl font-black text-slate-950">{selectedParticipant?.completed ? "완료" : "진행 중"}</p>
            </div>
          </div>
        </div>

        <div className="space-y-6">
          <TotalRankCards result={selectedResult} />
          <MetricRankList result={selectedResult} />
        </div>
      </section>
    </main>
  );
...
export default function App() {
  ...
  const startEvaluation = async () => {
    ...
    try {
      ...
      console.error(error);
      alert("평가 시작 중 오류가 발생했습니다. Firebase 연결과 Firestore Rules를 확인해주세요.");
    }
  };
  ...
  const updateParticipantAnswers = (nextAnswers) => {
    setParticipants((prev) => {
      ...
      if (existingIndex < 0) {
        return [...prev, { evaluatorId, evaluatorKey, answers: nextAnswers }];
      }

      const next = [...prev];
      next[existingIndex] = { ...next[existingIndex], answers: nextAnswers };
      return next;
    });
  };
  ...
  if (screen === "results") {
    return (
      <ResultsScreen
        ...
        onParticipantView={() => setScreen("participant")}
      />
    );
  }
