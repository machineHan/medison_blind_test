...
import { isGoogleSheetsEnabled } from "./surveyDb";
import {
      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[1.08fr_0.92fr] lg:px-8 lg:py-12">
        <div>
          <div className="mb-6 flex items-start justify-between gap-4 sm:mb-10">
            ...
            <div className="hidden text-right text-sm font-semibold text-slate-500 md:block">Medical Artificial<br />Intelligence Laboratory</div>
          </div>
        ...
        <div className="self-center rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-xl shadow-slate-200/70 sm:rounded-[2rem] sm:p-7">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full bg-[#eaf4f7] px-4 py-2 text-sm font-extrabold text-[#0f5d75]">
            <ClipboardCheck size={16} /> Google Sheets Collection
          </div>
          ...
          <div className="mt-7 grid grid-cols-1 gap-3 sm:grid-cols-3">
            <div className="rounded-2xl bg-slate-50 p-4 text-center"><p className="text-3xl font-black">{questions.length}</p><p className="mt-1 text-xs font-bold text-slate-500">평가 샘플</p></div>
            <div className="rounded-2xl bg-slate-50 p-4 text-center"><p className="text-3xl font-black">3</p><p className="mt-1 text-xs font-bold text-slate-500">Candidates</p></div>
            <div className="rounded-2xl bg-slate-50 p-4 text-center"><p className="text-3xl font-black">{metrics.length}</p><p className="mt-1 text-xs font-bold text-slate-500">평가 항목</p></div>
          </div>

          <div className="mt-5 rounded-2xl bg-slate-50 p-4 text-sm font-bold text-slate-600">
            저장 모드: {isGoogleSheetsEnabled ? "Google Sheets" : "localStorage"} · 등록 평가자 {participantCount}명
          </div>
  ...
  const onScore = async (questionId, candidateId, metricId, score) => {
    ...
    try {
      ...
      await saveMetricScore({
        evaluatorId,
        questionId,
        sampleTitle: question.sampleTitle,
        candidateId,
        candidateLabel: candidateList.find((item) => item.id === candidateId)?.label || candidateId,
        model: answerKey[questionId][candidateId],
        metricId,
      ...
      <div className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-2 py-2 sm:px-6 sm:py-4">
          ...
          <div className="flex shrink-0 items-center gap-1.5 sm:gap-3">
            ...
            <button onClick={onReset} className="rounded-xl border border-slate-200 bg-white px-2.5 py-2 text-xs font-black text-slate-600 sm:rounded-2xl sm:px-4 sm:py-3 sm:text-sm"><RotateCcw size={14} /></button>
          </div>
        </div>
      </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm font-bold text-slate-500">집계 대상 평가자 {sourceParticipants.length}명 · 저장 모드 {isGoogleSheetsEnabled ? "Google Sheets" : "localStorage"}</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button onClick={() => setScope("all")} className={cn("rounded-2xl px-4 py-3 text-sm font-black", scope === "all" ? "bg-slate-950 text-white" : "bg-white text-slate-600")}>전체 응답</button>
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[1fr_0.9fr]">
          <div className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm">
            <div className="mb-5 flex items-center gap-2 text-lg font-black text-slate-950"><ClipboardCheck size={20} /> 평가 항목별 평균 점수</div>
            <div className="space-y-5">
              {result.metricRows.map((row) => (
                <div key={row.metric.id} className="rounded-2xl border border-slate-100 p-4">
                  <h3 className="text-base font-black text-slate-950">{row.metric.label}</h3>
                  <div className="mt-4 space-y-3">
                    {row.scores.map((score) => (
                      <div key={score.method} className="grid grid-cols-[92px_1fr_50px] items-center gap-2 sm:grid-cols-[130px_1fr_60px] sm:gap-3">
                        <p className="truncate text-sm font-black text-slate-700">{score.method}</p>
                        <ScoreBar value={score.average} />
                        <p className="text-right text-sm font-black text-slate-950">{score.average.toFixed(2)}</p>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm">
            <div className="mb-5 flex items-center gap-2 text-lg font-black text-slate-950"><Eye size={20} /> 항목별 모델 점수 나열</div>
...
export default function App() {
  ...
  useEffect(() => {
    const unsubscribe = subscribeParticipants((items) => setParticipants(items));
    return () => unsubscribe?.();
  }, []);
  ...
  const updateParticipantAnswers = (nextAnswers) => {
    setParticipants((prev) => {
      ...
      if (existingIndex < 0) return [...prev, { evaluatorId, evaluatorKey, answers: nextAnswers }];
      const next = [...prev];
  ...
  const reset = () => {
    const ok = window.confirm("현재 화면의 입력 상태만 초기화할까요? 이미 저장된 Google Sheets/localStorage 데이터는 유지됩니다.");
    if (!ok) return;
    setAnswers({});
    setScreen("start");
  };
  ...
  if (screen === "start") {
    return <StartScreen evaluatorId={evaluatorId} setEvaluatorId={setEvaluatorId} onStart={startEvaluation} onResults={() => setScreen("results")} participantCount={participants.length} />;
  }

  if (screen === "results") {
    return <ResultsScreen participants={participants} currentEvaluatorId={evaluatorId} currentAnswers={answers} onBack={() => setScreen(evaluatorId ? "eval" : "start")} />;
  }

  return <EvaluationScreen evaluatorId={evaluatorId.trim()} answers={answers} setAnswers={setAnswers} onResults={() => setScreen("results")} onReset={reset} onParticipantAnswers={updateParticipantAnswers} />;
}
